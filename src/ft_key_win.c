...
void ft_print_menu(t_mlx *mlx)
{
	...
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 130, 300, RED,  ft_itoa(mlx->map->sz/32));
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 5,  315, RED,  "offset_y = " );
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 130, 315, RED,  ft_itoa(mlx->camera->offset_y));
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 5,  330, RED,  "maxz = " );
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 130, 330, RED,  ft_itoa(mlx->map->max_z));
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 5,  345, RED,  "minz = " );
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 130, 345, RED,  ft_itoa(mlx->map->min_z));
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 5,  360, RED,  "midz = " );
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 130, 360, RED,  ft_itoa(mlx->map->mid_z));
	mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 5,  375, RED,  "zoom = " );
    mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 130, 375, RED,  ft_itoa(mlx->camera->zoom + 36));
    mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 5,  390, RED,  "PROJECTION = " );
    mlx_string_put(mlx->wnd->ptr, mlx->wnd->wnd, 130, 390, RED,  ft_itoa(mlx->camera->projection));
}
